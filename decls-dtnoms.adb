package body decls.dtnoms is
    ...
    procedure posa(tc: in out taula_caracters; idx_tc: in out index_tcaracters; s: in string) is
    begin
    ...
    begin    
        p := hash(s);
        idx := tdispersio(p);
        while idx /= id_nom'first and then not compara(tcaracters, s, tblocs(idx).ptcaracters) loop
    ...
    function con(tc: in taula_caracters; idx: in index_tcaracters) return index_tcaracters is
        i : index_tcaracters;
    begin
        i := idx;
