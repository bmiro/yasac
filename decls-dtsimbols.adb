...
package body decls.dtsimbols is
	...
	procedure posa(ts: in out tsimbols; id: in id_nom; d: in descripcio; error: out boolean) is
		ne : id_nom;	
		td: tdescripcions renames ts.td;
		ta: tambits renames ts.ta;
	...
	procedure posa_camp(ts: in out tsimbols; idr, idc: in id_nom; d: in descripcio; error: out boolean) is
		dr: descripcio;	
		ie, ne: id_nom;
		td: tdescripcions renames ts.td;
		ta: tambits renames ts.ta;
	...
	begin
 	...
 	function cons_camp(ts: in tsimbols; idr, idc: in id_nom) return descripcio is
		dr: descripcio;	
		dc: descripcio(d_nul);
		ie: id_nom;
		td: tdescripcions renames ts.td;
	...
	procedure posa_param(ts: in out tsimbols; idproc, idparf: in id_nom; dparf: in descripcio; dperf: out boolean) is
		dproc : descripcio;
		ie, pie, ne : id_nom;	
		td: tdescripcions renames ts.td;		
		te: texpansio renames ts.te;
	...
	function primer_param(ts: in tsimbols; idproc: in id_nom) return it_param is
		d : descripcio;
		td: tdescripcions renames ts.td;	
	begin
		d := td(idproc).d;
	...
	procedure cons_param(ts: in tsimbols; it: in it_param; idparf: out id_nom; dparf: out descripcio) is
		te: texpansio renames ts.te;	
	begin
